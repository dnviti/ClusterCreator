...
check_required_python_libs() {
  for lib in "$@"; do
    if ! python3 -c "import $lib" &> /dev/null; then
      echo -e "${RED}Error: Python library '$lib' is required but not installed.${ENDCOLOR}"
      echo -e "${YELLOW}Please install it by running: pip install $lib${ENDCOLOR}"
      exit 1
    fi
  done
}
export -f check_required_python_libs
...
display_usage() {
    ...
    echo "  get-repo-path             Prints the configured repository path"
    ...
    echo "  install-system-services   Installs all configured system services to an existing cluster"
    echo "  uninstall-system-services Uninstalls all system services from the cluster"
    echo "  install-monitoring        Installs the configured monitoring stack"
    echo "  uninstall-monitoring      Uninstalls the monitoring stack"
    echo "  install-user-addons       Installs all configured user addons to an existing cluster"
    echo "  uninstall-user-addons     Uninstalls all user addons from the cluster"
...
check_required_python_libs "${required_python_libs[@]}"
...
if [[ "$COMMAND" != "" && "$COMMAND" != "setup-ccr" && "$COMMAND" != "-h" && "$COMMAND" != "--help" ]]; then
    ...
    if [ -f "$REPO_PATH_FILE" ]; then
        ...
        echo -e "${RED}Repository path not set. Run './clustercreator.sh setup-ccr' to set this value.${ENDCOLOR}" >&2
        exit 1
    fi
...
if [[ "$COMMAND" == "template" || \
      "$COMMAND" == "bootstrap" || \
      "$COMMAND" == "install-system-services" || \
      "$COMMAND" == "uninstall-system-services" || \
      "$COMMAND" == "install-monitoring" || \
      "$COMMAND" == "uninstall-monitoring" || \
      "$COMMAND" == "install-user-addons" || \
      "$COMMAND" == "uninstall-user-addons" || \
      "$COMMAND" == "add-nodes" || \
      "$COMMAND" == "drain-node" || \
      "$COMMAND" == "delete-node" || \
...
case "$COMMAND" in
    ...
    install-system-services)
        ( "$REPO_PATH/scripts/install-system-services.sh" "$@" )
        ;;
    uninstall-system-services)
        ( "$REPO_PATH/scripts/uninstall-system-services.sh" "$@" )
        ;;
    install-monitoring)
        ( "$REPO_PATH/scripts/install-monitoring.sh" "$@" )
        ;;
    uninstall-monitoring)
        ( "$REPO_PATH/scripts/uninstall-monitoring.sh" "$@" )
        ;;
    install-user-addons)
        ( "$REPO_PATH/scripts/install-user-addons.sh" "$@" )
        ;;
    uninstall-user-addons)
        ( "$REPO_PATH/scripts/uninstall-user-addons.sh" "$@" )
        ;;
    add-nodes)
    ...
    tofu)
        ...
        ;;
    must-gather)
        ( "$REPO_PATH/scripts/must-gather.sh" "$@" )
        ;;
    *)
