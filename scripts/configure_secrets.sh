...
env_variables=(
    ...
    "GITHUB_AUTH_TOKEN|Enter your GitHub Personal Access Token (PAT) with repo scope"
    "GITEA_RUNNER_TOKEN|Enter your Gitea Runner Registration Token (from Site Administration > Runners)"
)
...
variable "github_auth_token" {
  default = "$(grep -m 1 "^GITHUB_AUTH_TOKEN=" "$TMP_ENV_FILE" | sed -E 's/^[^=]+=\s*"([^"]*)".*$/\1/')"
}
variable "gitea_runner_token" {
  default = "$(grep -m 1 "^GITEA_RUNNER_TOKEN=" "$TMP_ENV_FILE" | sed -E 's/^[^=]+=\s*"([^"]*)".*$/\1/')"
}
variable "vm_ssh_key" {
  type = list(string)
  default = [
