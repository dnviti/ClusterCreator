...
echo -e "${GREEN}Checking network connectivity before package installation...${ENDCOLOR}"
if ! sudo virt-customize -a "$PROXMOX_ISO_PATH"/"$IMAGE_NAME" --run-command "ping -c 4 1.1.1.1"; then
    echo -e "${RED}Network connectivity check failed. Aborting template creation.${ENDCOLOR}"
    exit 1
fi
...
echo -e "${GREEN}Verifying installation of essential commands...${ENDCOLOR}"
for cmd in kubectl kubeadm containerd; do
    if ! sudo qm guest exec "$TEMPLATE_VM_ID" -- /bin/sh -c "command -v $cmd"; then
        echo -e "${RED}Warning: Command '$cmd' not found in the template. The template may not be fully functional.${ENDCOLOR}"
    else
        echo -e "${GREEN}Command '$cmd' found.${ENDCOLOR}"
    fi
done
